...
import shap
import torch
from Explain import getShapValue

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run multimodal mobility nowcasting.')

    # command line argumente
    parser.add_argument('-device', '--device', type=str, help='Specify device usage', default='cpu',
                        choices=['cpu', 'cuda:0', 'cuda:1', 'cuda:2', 'cuda:3'])
    parser.add_argument('-in', '--input_dir', type=str, default='./data')
    # dataset, model
    parser.add_argument('-data', '--data', type=str, help='Specify data', default='JONAS-DC', choices=['JONAS-NYC', 'JONAS-DC', 'COVID-CHI', 'COVID-US'])
    parser.add_argument('-model', '--model', type=str, help='Specify model', default='EAST-Net', choices=['ST-Net', 'EAST-Net'])
    ...
    parser.add_argument('-epoch', '--num_epochs', type=int, default=50)
    parser.add_argument('-test', '--test_only', default = 'Train',  action='store_true')
    ...
    params['data_dir'] = '/content/drive/MyDrive/EAST-Net/data/JONAS-DC-9x12-20151024-20160131-change.npz'
    params['output_dir'] = '/content/drive/MyDrive/EAST-Net/output/'
    # load data
    ...
    trained_checkpoint = torch.load(params['output_dir']+f'/{params["model"]}-{params["data"]}.pkl', map_location='cpu')
    model = trainer.get_model()
    model.load_state_dict(trained_checkpoint['state_dict'])
    data = next(iter(data_loader['test']))
    ht_pair = (torch.zeros(params['batch_size'], params['H'] * params['W'], params['hidden_dim']),
                               torch.zeros(params['batch_size'], params['C'], params['hidden_dim']))
    shap_X, shap_tx, shap_ty = getShapValue(model, data, ht_pair, data_input)

    shap_str = f"SHAP value for x_seq: {shap_X}\n"
    shap_str += f"SHAP value for t_x: {shap_tx}\n"
    shap_str += f"SHAP value for t_y: {shap_ty}\n"

    # 将字符串写入到文件中
    with open("shap_values.txt", "w") as file:
        file.write(shap_str)
